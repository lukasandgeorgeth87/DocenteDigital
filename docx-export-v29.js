...
(function(){
  ...
  function docxBlob(title,html,landscape=false){
    ...
    const documentXml=`<?xml version="1.0" encoding="UTF-8" standalone="yes"?><w:document xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"><w:body>${body}<w:p>${runXml(`${state.teacherName||'Docente'} · ${state.schoolName||'Institución Educativa'}`,false,18)}</w:p><w:sectPr>${pgSz}<w:pgMar w:top="720" w:right="720" w:bottom="720" w:left="720" w:header="360" w:footer="360" w:gutter="0"/></w:sectPr></w:body></w:document>`;
