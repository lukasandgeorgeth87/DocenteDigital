...
(function(){
  ...
  function situationOptions(raw){
    ...
    let a='',b='';
    const typedTitle=tidy(byId('unitTitle')?.value||'');
    const combinedHeritage=tidy(raw+' '+focus+' '+typedTitle);
    const heritage=/pinturas?\s+rupestres?|arte\s+rupestre|petroglif|restos?\s+arqueologic|sitios?\s+arqueologic|patrimonio\s+arqueologic/.test(combinedHeritage.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,''));
    const titlePlaceMatch=typedTitle.match(/\ben\s+([\p{L}][\p{L}'’ -]{2,80}(?:\s+y\s+[\p{L}][\p{L}'’ -]{2,50})?)\s*$/iu);
    const titlePlace=titlePlaceMatch?tidy(titlePlaceMatch[1]):'';

    if(heritage){
      const hasArchaeology=/restos?\s+arqueol[oó]gic|sitios?\s+arqueol[oó]gic|patrimonio\s+arqueol[oó]gic/i.test(combinedHeritage);
      const placePhrase=titlePlace?(' de '+titlePlace):' de nuestro territorio';
      const heritageFocus=hasArchaeology?('los restos arqueológicos y las pinturas rupestres'+placePhrase):('las pinturas rupestres'+placePhrase);
      const heritageWhere=profile.community
        ? ((profile.localityType||'localidad')+' '+profile.community)
        : (titlePlace||locality);
    ...
    if(level==='Inicial'){
      ...
      a=`En ${institution}, ubicada en ${heritageWhere}, los estudiantes de ${grades} abordarán ${focus} a partir de una situación vinculada con su realidad y con información verificable. Recuperarán experiencias y saberes previos, formularán preguntas que puedan investigarse, analizarán datos, fuentes o casos, contrastarán distintas explicaciones y construirán una posición o respuesta sustentada. ${facts||'Cuando sea necesario, el docente incorporará datos locales reales antes de presentar afirmaciones como hechos.'} El reto central será: ${reto}`;
