...
(function(){
  ...
  function territorialPhrase(){const c=state.teacherContext||{},name=(c.locationName||c.community||'').trim(),type=(c.locationType||'Localidad').trim().toLowerCase();if(name)return `${type} ${name}`;if((c.district||'').trim())return `distrito de ${c.district.trim()}`;return 'entorno de los estudiantes';}
  function summary(){const c=state.teacherContext||{};return [territorialLabel(),c.areaType&&c.areaType!=='No especificada'?`Área ${c.areaType}`:'',c.district,c.province,c.region,c.calendar,c.notes].filter(Boolean).join(' · ');}
  ...
  function neutralizeLegacyLabels(){
    const ta=document.getElementById('unitSituation');
    if(ta){
      ta.placeholder='Ejemplo: describe un hecho, necesidad, problema, oportunidad o práctica relevante de tu entorno';
      const small=ta.parentElement?.querySelector('small');
      if(small)small.textContent='Escribe la situación con naturalidad. La app analizará el contexto, lo que sí está expresado y el nivel de claridad antes de proponer una situación significativa, reto y productos.';
    }
    const materialTopic=document.querySelector('#materials input[value="Cuidado del agua en nuestra comunidad"]');
    if(materialTopic)materialTopic.value='Tema o situación vinculada con el entorno de los estudiantes';
  }
  ...
  function refreshQuick(){const box=document.getElementById('ddQuickContext');if(!box)return;box.innerHTML=`<b>📍 Contexto que usará esta planificación:</b> ${E(summary())} <button type="button" class="btn ghost dd-mini" id="ddEditContext">Editar contexto</button>`;const b=document.getElementById('ddEditContext');if(b)b.onclick=()=>{if(typeof go==='function')go('settings');setTimeout(()=>document.getElementById('ddTeacherContextSettings')?.scrollIntoView({behavior:'smooth'}),50);};}
  window.ddTerritorialLabel=territorialLabel;window.ddTerritorialPhrase=territorialPhrase;window.ddTerritorialSummary=summary;
  const oldGo=window.go;if(typeof oldGo==='function')window.go=function(id){const r=oldGo.apply(this,arguments);setTimeout(()=>{if(id==='settings')mount();neutralizeLegacyLabels();refreshQuick();},0);return r;};
  const oldShow=window.showUnit;if(typeof oldShow==='function')window.showUnit=function(){const r=oldShow.apply(this,arguments);setTimeout(()=>{neutralizeLegacyLabels();refreshQuick();},0);return r;};
  setTimeout(()=>{mount();neutralizeLegacyLabels();refreshQuick();},0);
})();